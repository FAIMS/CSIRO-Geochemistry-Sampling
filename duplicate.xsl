...
<xsl:if test="name() = 'input' or name() = 'select1'">makeVocab(attributes, "<xsl:value-of select="normalize-space(@faims_attribute_name)"/>", <xsl:copy-of select="$path" />, <xsl:if test="name() = 'select1'">false</xsl:if><xsl:if test="name() = 'input'">true</xsl:if>);
</xsl:if>
