...
attributes.add("<xsl:value-of select="normalize-space(@faims_attribute_name)"/>", getFieldAnnotation(<xsl:copy-of select="$path" />), <xsl:if test="name() = 'select' or name() = 'select1'">getFieldValue(<xsl:copy-of select="$path" />)</xsl:if><xsl:if test="name() != 'select' and name() != 'select1'">null</xsl:if>, <xsl:if test="name() = 'input'">getFieldValue(<xsl:copy-of select="$path" />)</xsl:if><xsl:if test="name() != 'input'">null</xsl:if>, getFieldCertainty(<xsl:copy-of select="$path" />));</xsl:for-each> 
</xsl:template>
